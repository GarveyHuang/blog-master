	...
	var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
		render: function(target, container, frozen){
			...
			for(var i=0; i<groups.length; i++){
				...
				table.push('<div class="datagrid-group" group-index=' + i + ' styles="height:25px;overflow:hidden;border-bottom:1px solid #ccc;">');
				table.push('<table cellspacing="0" cellpadding="0" border="0" styles="height:100%"><tbody>');
				table.push('<tr>');
				table.push('<td styles="border:0;">');
				if (!frozen){
					table.push('<span styles="color:#666;font-weight:bold;">');
				...
				for(var j=0; j<group.rows.length; j++) {
					// get the class and styles attributes for this row
					...
					var style = styleValue ? 'styles="' + styleValue + '"' : '';
		...
		onAfterRender: function(target){
			...
			$('<td styles="border:0;text-align:center;width:25px"><span class="datagrid-row-expander datagrid-row-collapse" styles="display:inline-block;width:16px;height:16px;cursor:pointer">&nbsp;</span></td>').insertBefore(group.find('td'));
